...
setInterval(() => {
  ...
  console.log(
    `[STATUS] Total peer IDs registered: ${Object.keys(peerIds).length}`
  );
}, 30000);
...
io.on("connection", (socket) => {
  ...
  console.log(
    `  Current peers in room: ${currentPeerCount}/${MAX_PEERS_PER_ROOM}`
  );

  // Limitar a 2 usuarios por sala
  if (currentPeerCount >= MAX_PEERS_PER_ROOM) {
    console.log(
      `  ❌ REJECTED - Room ${roomId} is FULL (${currentPeerCount}/${MAX_PEERS_PER_ROOM})`
    );
  ...
  console.log(
    `[USER_COUNT] Broadcasted to room ${roomId}: ${rooms[roomId].size} users`
  );
  ...
  socket.on("registerPeerId", (peerId: string) => {
    ...
    const otherPeersInRoom = Array.from(rooms[roomId]).filter(
      (id) => id !== socket.id
    );
    ...
    if (otherPeersInRoom.length > 0) {
      otherPeersInRoom.forEach((otherId) => {
        ...
        if (otherPeerId) {
          ...
          console.log(
            `  📤 Sent to new peer (${peerId.substring(
              0,
              8
            )}...): remote peer ID ${otherPeerId.substring(0, 8)}...`
          );

          // Enviar al usuario existente el Peer ID del nuevo usuario
          io.to(otherId).emit("remotePeerId", peerId);
          console.log(
            `  📤 Sent to existing peer (${otherPeerId.substring(
              0,
              8
            )}...): remote peer ID ${peerId.substring(0, 8)}...`
          );

          console.log(
            `  ✅ PEER EXCHANGE COMPLETE between ${peerId.substring(
              0,
              8
            )}... and ${otherPeerId.substring(0, 8)}...`
          );
        } else {
          console.log(
            `  ⚠️ Other peer ${otherId.substring(
              0,
              8
            )}... has no registered Peer ID yet`
          );
        }
      });
    } else {
      console.log(
        `  ⏳ WAITING - ${peerId.substring(
          0,
          8
        )}... is the only peer in the room`
      );
    }
  });
  ...
  socket.on("signal", (to, from, data) => {
    ...
    console.log(
      `[SIGNAL] From ${from.substring(0, 8)}... to ${to.substring(0, 8)}...`
    );
  });
  ...
  socket.on("disconnect", () => {
    ...
    if (rooms[roomId]) {
      ...
      console.log(
        `  Remaining peers in room: ${rooms[roomId].size}/${MAX_PEERS_PER_ROOM}`
      );
